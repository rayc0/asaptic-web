...
- `demo/proof-lab/index.html` — the two-panel proof surface (no-build, self-contained, `<meta name="robots" content="noindex">`).
- `demo/proof-lab/prooflab.mjs` — client-side gate + match logic, imported by the page via `<script type="module">`, unit-testable headless.
- `demo/proof-lab/prooflab.test.mjs` — `node --test` suite (23 tests incl. moat sentinel).
...
3. **Robots** — page is intentionally `noindex`. No sitemap entry needed. If a global sitemap generator enumerates `demo/**`, exclude `demo/proof-lab/`.
