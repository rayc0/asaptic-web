...
const CONTACT_EMAIL = 'engage@asaptic.com';
...
export function parseFilters(input, snapshot) {
  ...
  const q = get('q');
  if (q !== undefined) f.q = String(q).toLowerCase();
...
export const TENDER_TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      'plus optional markets, categories, and a note. A human follows up by email.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        categories: { type: 'string', description: 'Categories or listing ids of interest.' },
  ...
  try {
    switch (name) {
      ...
      case 'request_tender_access': {
        ...
        // Honest acknowledgment: no fabricated reference number; persistence
        // and automated intake land in a later phase, so the guaranteed path
        // is named explicitly.
        return {
          result: {
            acknowledged: true,
