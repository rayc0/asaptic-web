...
export async function explainFit(env, params, { defaultTopN = EXPLAIN_DEFAULT_TOPN } = {}) {
  ...
  // Single-row mode: explain the fit for one requested public listing.
  const rawId = params.asaptic_id;
...
export const TENDER_TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      'does not advise on bidding, pricing, or how to win. With asaptic_id it explains that one listing; ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        asaptic_id: { type: 'string', description: 'Optional AT-… id: explain the fit for that one listing instead of ranking.' },
